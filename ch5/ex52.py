class DoublyLinkList:
    ...
    def __str__(self):
        if self.isEmpty():
            return "Empty"
        cur, s = self.head, str(self.head.value) + " "   #ค่าของหัว หัวต่อ
        while cur.next != None:                  #ต่อกัน?
    ...
    def reverse(self):
        ...
        if self.size == 1:
            x = self.head.value
            return str(x)
        cur, s = self.tail, str(self.tail.value) + " "   #เอาหางมาต่อ
        while cur.prev != None:
    ...
    def append(self, i):
        if (self.head == None):
            ...
            q = self.nodeAt(self.size - 1) #ต่อท้าย?
    ...
    def indexOf(self, data): #หาตำแหน่งจากค่าที่ป้อนไป
        q = self.head
...
for i in range(len(inp)):
    if (inp[i][:2] == "AP"):
        L.append(inp[i][3:]) #คือ input จริงๆ
    elif (inp[i][:2] == "AH"):
        L.addHead(inp[i][3:])
    elif (inp[i][:2] == "SE"):
